...
export default function TeacherClassroomView({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeleteClassroom = async () => {
    if (confirm("Sind Sie sicher, dass Sie diesen Classroom löschen möchten?")) {
      try {
        ...
        alert('Fehler beim Löschen des Classrooms')
      }
    }
  }

  const handleFinalizeClassroom = async () => {
    if (confirm("Sind Sie sicher, dass Sie diesen Classroom finalisieren möchten? Nach der Finalisierung können keine Änderungen mehr vorgenommen werden.")) {
      try {
  ...
  const handleSearchStudent = async (name: string) => {
    ...
    setTimeout(() => {
      ...
      const existingUsers = ['Max Mustermann', 'Anna Schmidt', 'Tom Weber', 'Lisa Müller']
